  ...
  const fetchFunds = async (category: string | null = null) => {
    try {
      ...
      if (category) {
        ...
        response = await fetch("/api/filter-category", {
          method: "POST",
          headers: {
        ...
        response = await fetch("/api/funds/limited");
      }
      ...
      <div className="min-h-[60vh] text-center bg-[url(../../assets/mainpage-banner.jpg)] bg-cover z-0">
        <div className="z-10 min-h-[60vh] flex flex-col items-center justify-center text-center bg-transparent backdrop-blur-[10px] backdrop-saturate-121 glass">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-10">
            {[
              { text: "Campaigns", icon: "✊", category: "Campaigns" },
              { text: "Business", icon: "💡", category: "Business" },
              { text: "Community", icon: "🫖", category: "Community" },
              { text: "Charity", icon: "💚", category: "Charity" },
              { text: "Personal fundraisers", icon: "🌱", category: "Personal fundraisers" },
              { text: "Sports", icon: "⚽", category: "Sports" },
              { text: "Prize draws", icon: "🎟️", category: "Prize draws" },
              { text: "Community shares", icon: "⏳", category: "Community shares" },
            ].map((item, index) => (
              <button
                key={index}
                onClick={() => handleCategoryClick(item.category)}
                className={`flex items-center space-x-2 p-2 rounded-lg transition-colors ${
                  selectedCategory === item.category ? "bg-blue-500 text-white" : "bg-transparent text-white hover:bg-gray-700/50"
                }`}
              >
